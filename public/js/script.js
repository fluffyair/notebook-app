...
function delNote(btn) {
    ...
    console.log(btn);
        ...
        for (let i = 0; i < ob2.length; i++) {
            ...
            if(splitTitle[1] == `(0)`) html += `<section><h1>${ob2[i].title + ` (${i - 1})`}</h1><p>${ob2[i].description}</p><button onclick="delNote(this)" class="trash"><i class="fa-solid fa-trash"></i></button></section>` 
            else
            html += `<section><h1>${ob2[i].title + ` (${i})`}</h1><p>${ob2[i].description}</p><button onclick="delNote(this)" class="trash"><i class="fa-solid fa-trash"></i></button></section>`
